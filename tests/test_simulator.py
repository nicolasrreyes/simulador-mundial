...
def test_simulator_run_champion_comes_from_semifinalists(client: TestClient):
    ...
    data = res.json()
    semifinalists = set()
    for sf in data["semifinals"]:
        semifinalists.add(sf["home_team"])
        semifinalists.add(sf["away_team"])
    assert data["champion"] in semifinalists


def test_simulator_run_auto_codes_dont_collide_with_manual(client: TestClient):
    client.post("/teams/", json={"name": "Manual One", "code": "P01"})
    client.post("/teams/", json={"name": "Manual Two", "code": "P02"})
    res = client.post("/simulator/run")
    assert res.status_code == 200
    teams_res = client.get("/teams/")
    codes = [t["code"] for t in teams_res.json()]
    assert len(codes) == len(set(codes))


def test_simulator_run_third_place_are_semifinal_losers(client: TestClient):
    res = client.post("/simulator/run")
    data = res.json()
    if data.get("third_place"):
        sf_losers = set()
        for sf in data["semifinals"]:
            loser = sf["away_team"] if sf["winner"] == sf["home_team"] else sf["home_team"]
            sf_losers.add(loser)
        tp_teams = {data["third_place"]["home_team"], data["third_place"]["away_team"]}
        assert tp_teams == sf_losers


def test_simulator_run_standings_tiebreaker_gd_gf(client: TestClient):
    res = client.post("/simulator/run")
    data = res.json()
    for g in data["groups"]:
        for i in range(len(g["standings"]) - 1):
            ...
            nxt = g["standings"][i + 1]
            if curr["pts"] == nxt["pts"]:
                assert (curr["gd"], curr["gf"]) >= (nxt["gd"], nxt["gf"])
