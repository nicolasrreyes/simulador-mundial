...
def test_create_player_position_stored_uppercase(client: TestClient):
    team = _create_team(client)
    res = client.post("/players/", json={"name": "Messi", "position": "fw", "team_id": team["id"]})
    assert res.status_code == 201
    assert res.json()["position"] == "FW"


def test_update_player_team_id_only(client: TestClient):
    t1 = _create_team(client, "Argentina", "ARG")
    t2 = client.post("/teams/", json={"name": "Brasil", "code": "BRA"}).json()
    res = client.post("/players/", json={"name": "Messi", "position": "FW", "team_id": t1["id"]})
    pid = res.json()["id"]
    res = client.put(f"/players/{pid}", json={"team_id": t2["id"]})
    assert res.status_code == 200
    assert res.json()["team_id"] == t2["id"]


def test_create_player_empty_name_returns_422(client: TestClient):
    team = _create_team(client)
    res = client.post("/players/", json={"name": "", "position": "FW", "team_id": team["id"]})
    assert res.status_code == 422


def test_update_player_position_uppercased(client: TestClient):
    team = _create_team(client)
    res = client.post("/players/", json={"name": "Messi", "position": "FW", "team_id": team["id"]})
    pid = res.json()["id"]
    res = client.put(f"/players/{pid}", json={"position": "gk"})
    assert res.status_code == 200
    assert res.json()["position"] == "GK"